 ...
 * Add menu support
 */
add_action('init', 'jd_register_menu');
function jd_register_menu()
{
	register_nav_menus([
		'main' => esc_html__('Primary', '_juststart'),
	]);
}
 ...
 * Remove site icon
 */
function jd_remove_styles_sections()
{
	global $wp_customize;
...
add_action('customize_register', 'jd_remove_styles_sections', 20);
	...
	function tcd_restrict_admin_pages()
	{
		...
		foreach ($restricted_screens as $restricted_screen) {
			if ($current_screen_id === $restricted_screen) {
				wp_die(__('You are not allowed to access this page.', '_juststart'));
			}
		}
	}
}
